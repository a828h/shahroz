...
<script>
    function redirector(page = 0, perpage = 0) {
        ...
        var url = "{{route('admin.users.index')}}" + '?' + queryString;
        console.log("{{route('admin.users.index')}}" + '?' + queryString);
        if(page != 0) {
            url += '&page=' + page;
        }
    ...
    jQuery(document).ready(function () {
        $("#search_status, #search_role").selectpicker()
    ...
    $(".delete-user").click(function (e) {
        ...
        var id = $(this).data('id');
        swal.fire({
            title: "{{__('admin.users.are_you_sure')}}",
            text: "{{__('admin.users.are_you_sure_to_delete')}}",
            type: "warning",
            showCancelButton: !0,
